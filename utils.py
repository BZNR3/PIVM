...
class DDPMDataset(Dataset):
    """
    Dataset for DDPM-based paired image generation.

    When no valid previous slice exists (for example, the first slice of
    a case), the current image is used as the previous image.
    """

    def __init__(self, image_paths, label_paths, organ_paths, img_size, use_self_as_prev=False):
        self.image_paths = image_paths
        self.label_paths = label_paths
        self.organ_paths = organ_paths
        self.use_self_as_prev = use_self_as_prev

        # Transform for image and label inputs
    ...
    def __getitem__(self, index):
        ...
        current_slice = int(parts[-1])

        # Load current image slice
        x = Image.open(self.image_paths[index]).convert('L')
        if self.transform1:
            x = self.transform1(x)

        # Determine previous image slice path
        prev_slice = current_slice - 1
        dir_path = os.path.dirname(self.image_paths[index])
        prev_filename = os.path.join(dir_path, f"{case_id}_{prev_slice}.png")

        # If the previous slice does not exist, use the current slice itself
        if self.use_self_as_prev or prev_slice < 1 or not os.path.exists(prev_filename):
            prev_filename = self.image_paths[index]

        prev = Image.open(prev_filename).convert('L')
        if self.transform1:
            prev = self.transform1(prev)

        # Load paired label input
        y = Image.open(self.label_paths[index]).convert('L')
        if self.transform1:
            y = self.transform1(y)

        # Load organ mask
        z = Image.open(self.organ_paths[index]).convert('L')
        if self.transform2:
            z = self.transform2(z)

        return x, prev, y, z, filename
