...
class Up(nn.Module):
    ...
    def forward(self, x, skip_x=None, t=None, last=False):
        x = self.up(x) if not last else F.interpolate(
            x, size=x.shape[2] + 16, mode="bilinear", align_corners=True
        )
...
class DDPM(nn.Module):
    def __init__(self, time_dim=256, img_channels=3, dim=2 * 2048, depth=1, heads=4, dim_head=64,
    ...
    def forward(self, x, t, organ=None, prev_image=None):
        t = t.unsqueeze(-1).type(torch.float)
        t = self.pos_encoding(t, self.time_dim)

        if self.img_channels == 3:
            x = torch.cat([x, prev_image, organ], dim=1)
...
if __name__ == '__main__':
    ...
    ddpm = DDPM(img_channels=3).to(device)
    tensor_x = torch.randn(1, 1, IMAGE_SIZE, IMAGE_SIZE).to(device)
    tensor_prev = torch.randn(1, 1, IMAGE_SIZE, IMAGE_SIZE).to(device)
    tensor_organ = torch.randn(1, 1, IMAGE_SIZE, IMAGE_SIZE).to(device)
    tensor_t = tensor_x.new_tensor([500] * tensor_x.shape[0]).long().to(device)

    output = ddpm(tensor_x, tensor_t, organ=tensor_organ, prev_image=tensor_prev)
    print(output.shape)
